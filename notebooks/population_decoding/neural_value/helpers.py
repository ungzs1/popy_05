...
def process_fold(neural_dataset, fold_id, train_idx, test_idx, target):
    ...
    for train_t_id, t in enumerate(neural_dataset.time.values):
        # all data at this time (for projection)
        X_temp = neural_dataset.firing_rates.sel(time=t).values
        # get the firing rates for the current time point
        ...
        # balance dataset
        if not continuous:
            X_train_temp_balanced, y_train_temp_balanced = rus.fit_resample(X_train_temp, y_train)
            X_test_temp_balanced, y_test_balanced = rus.fit_resample(X_test_temp, y_test)
        else:  # fit on the original one (ugly implementation)
            X_train_temp_balanced, y_train_temp_balanced = X_train_temp, y_train
            X_test_temp_balanced, y_test_balanced = X_test_temp, y_test

        # fit the model on train data
        clf.fit(X_train_temp_balanced, y_train_temp_balanced)

        # evaluate on test set
        score = clf.score(X_test_temp_balanced, y_test_balanced)

        # project test set 
        coeffs = clf.coef_.squeeze()
        coeffs_norm = np.linalg.norm(coeffs)
        data_projected = X_temp @ coeffs.T / coeffs_norm  # normalize by the norm of the coefficients to get a more interpretable projection

        # Store score and projections
        scores_fold[train_t_id] = score
        projections_fold.append(data_projected.ravel())
    ...
    projections_fold = xr.DataArray(projections_fold.T, dims=('trial_id', 'time'), 
                                   coords={**trial_coords, **time_coords})
    
    return fold_id, scores_fold, projections_fold


def time_resolved_decoder_all_time_target_stratified(neural_dataset, target='R_1', group='target'):
    trial_ids = neural_dataset.trial_id.values
    labels = neural_dataset[target].values  
    groups = neural_dataset[group].values  # convert to string to avoid issues with GroupKFold
    cv = 10

    # create train and test splits
    if len(np.unique(neural_dataset[target].values)) > 2:
        raise ValueError("StratifiedKFold is not suitable for continuous targets. Please use time_resolved_decoder_all_time instead.")
    else:
        continuous = False
        skf = GroupKFold(n_splits=len(np.unique(groups)))
    ...
    results = {}
    for fold_id, (train_idx, test_idx) in enumerate(skf.split(trial_ids, labels, groups)):
        # SWAP TRAIN AND TEST IDS: we want to train on one group and test on the rest, not the other way around (which is the default)
        train_idx, test_idx = test_idx, train_idx
        train_group = groups[train_idx[0]]  # all train idx belong to the same group, so we can take the first one

        # project ALL trials to the decision boundary at each time point, get scores for each time point
        fold_id, scores_fold, projections_fold = process_fold(
            neural_dataset.copy("deep"),
            fold_id,
            train_idx,
            test_idx,
            target,
        ...
        # addpend to the results
        results[train_group] = (fold_id, scores_fold, projections_fold)

    # Collect results
    scores_all = np.full((n_timebins, cv), np.nan)  # (timebins, folds)

    for fold_id, scores_fold, projections_fold in results.values():
        scores_all[:, fold_id] = scores_fold
        
    # Concatenate projected data along a new dimension called 'subspace'. the value of the 'subspace' is the key of the dict.
    projected_data = {f'target_{str(int(key))}': value[2] for key, value in results.items()}
    projected_data = xr.concat(
        list(projected_data.values()),
        dim=pd.Index(list(projected_data.keys()), name='subspace'),
    )
    projected_data = projected_data.sortby('subspace')
...
def time_resolved_decoder_all_time(neural_dataset, target='R_1'):
    trial_ids = neural_dataset.trial_id.values
    labels = neural_dataset[target].values  
    cv = 10
    ...
    for fold_id, (train_idx, test_idx) in enumerate(skf.split(trial_ids, labels)):
        ...
        # Get only the projections of the test set
        projections_fold_across = projections_fold.sel(trial_id=neural_dataset.sel(trial_id=trial_ids[test_idx]).trial_id.values)

        # addpend to the results
        results.append(
            (fold_id, scores_fold, projections_fold_across)  # We dont need here the within fold projections, but we could return them if we want to analyze them separately
        )
    ...
    for fold_id, scores_fold, projections_fold in results:
        scores_all[:, fold_id] = scores_fold
...
def plot_across_time_decodability(projected_data=None, cmap='RdBu', vmin=None, vmax=None):
    """
    ...
    ax = axes[plot_idx]
    """ ACROSS-TIME DECODEABILITY MATRIX IS NOT THERE ANYMORE
