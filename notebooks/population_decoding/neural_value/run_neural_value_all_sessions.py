...
def run_neural_value_extraction(monkey, session, PARAMS):
    ...
    for subregion in np.unique(neural_dataset.subregion.data):
        ...
        #project neural data to the neural value space, get across-time decodability matrix
        data_projected = time_resolved_decoder_all_time(neural_dataset_temp, target='R_1')

        for t_interest_name, t_interest_value in PARAMS['times_of_interest'].items():
            # get neural value for the current trial and the previous one
            data_projected = add_neural_value_coord(data_projected, t_interest=t_interest_value)
            data_projected = data_projected.rename({'V_t': f'Vt_{t_interest_name}'})
            data_projected = data_projected.rename({'V_t_p1': f'Vtp1_{t_interest_name}'})
            data_projected = data_projected.rename({'dV': f'dV_{t_interest_name}'})
...
PARAMS = {
    'floc': os.path.join(cfg.PROJECT_PATH_LOCAL, 'notebooks', 'population_decoding', 'neural_value', 'results', 'behav_neural_value'),
    'msg': 'extracting plots',
    'times_of_interest': {'after_fb': (-4, -2), 
                            'it1': (-2, 0),
                            'it2': (-1, 1),
                            'before_fb': (1.5, 3.5)},
}
