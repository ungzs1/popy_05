...
def time_resolved_decoder_all_time(neural_dataset, target='R_1', group=None, t_project=None):
    trial_ids = neural_dataset.trial_id.values
    labels = neural_dataset[target].values  
    cv = 10
...
def add_neural_value_coord(data_projected, t_interest=[2.5, 3.5]):
    if isinstance(t_interest, float):
        V_t = data_projected.sel(time=t_interest, method='nearest').values
        V_t_p1 = data_projected.sel(time=t_interest+7.5, method='nearest').values
    elif isinstance(t_interest, list):
        V_t = data_projected.sel(time=slice(t_interest[0], t_interest[1])).mean('time').values
        V_t_p1 = data_projected.sel(time=slice(t_interest[0]+7.5, t_interest[1]+7.5)).mean('time').values
    dV = V_t_p1 - V_t

    data_projected = data_projected.assign_coords(V_t=('trial_id', V_t))
    data_projected = data_projected.assign_coords(V_t_p1=('trial_id', V_t_p1))
    data_projected = data_projected.assign_coords(dV=('trial_id', dV))
...
def plot_projected_data(data_projected, t_interest_fit, t_interest_value, normalize=False, n_extra_trials=(0, 1), xlim=None, paper_format=False):
    # project to axis
    if paper_format:
    ...
    ax.axvline(t_interest_fit, linestyle='dashed', label='time of fit', color='tab:red')
